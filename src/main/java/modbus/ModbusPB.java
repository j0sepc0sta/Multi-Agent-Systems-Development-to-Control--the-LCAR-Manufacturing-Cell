...
public class ModbusPB {
    public static void modbusPB(boolean status) {
        ...
        try (PlcConnection plcConnection = PlcDriverManager.getDefault().getConnectionManager().getConnection(connection)) {
            ...
            writeBuilder.addTagAddress("M82", "coil:83", status);
            PlcWriteRequest writeRequest = writeBuilder.build();
            PlcWriteResponse writeResponse = writeRequest.execute().get();
            if (writeResponse.getResponseCode("M82") == PlcResponseCode.OK) {
                System.out.println("Successfully wrote to AgentPB:"+status);
            } else {
                System.out.println("Failed to write to AgentPB. Response code: " + writeResponse.getResponseCode("M82"));
            }
